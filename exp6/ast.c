...
{ //处理变量列表
    ...
    case EXT_DEC_LIST:
        ...
        break;
    case ID:
        rtn = fillSymbolTable(T->type_id, LEV, T->type, 'V', 0); //最后一个变量名
        if (rtn == -1) {
    ...
    case ARRAYS_TERM:
        // printf("!\n");
        rtn = fillSymbolTable(T->type_id, LEV, T->type, 'V', 1); //最后一个变量名
        if (rtn == -1) {
            // case_1.sy:15 redefinition of ‘foo’
            int i;
            for (i = symbolTable.index - 1; i >= 0; i--)
    ...
    case VAR_DEC:
        if (T->ptr[0]->kind == ARRAYS_TERM) {
            rtn = fillSymbolTable(T->type_id, LEV, T->type, 'V', 1); //此处偏移量未计算，暂时为0
        } else {
            rtn = fillSymbolTable(T->type_id, LEV, T->type, 'V', 0); //此处偏移量未计算，暂时为0
        }
        if (rtn == -1)
            printf("%s:%d redefinition of ‘%s’\n", filename, T->pos, T->type_id);
    }
...
{ //处理基本表达式，参考文献[2]p82的思想
    ...
    if (T) {
        ...
        case ASSIGN:
            ...
            if (symbolTable.symbols[rtn].isArray && T->ptr[1]->kind != ARRAY_VAL) {
            ...
            } else if (T->ptr[0]->kind == ARRAYS_TERM) {
                Exp(T->ptr[0]);
                Exp(T->ptr[1]);
                T->type = T->ptr[0]->type;
            }
...
{ //对抽象语法树的先根遍历,按display的控制结构修改完成符号表管理和语义检查和TAC生成（语句部分）
    ...
    if (T) {
        ...
        case PARAM_LIST: //处理函数形式参数列
            ...
            break;
        case PARAM_DEC:
            rtn = fillSymbolTable(T->ptr[1]->type_id, 1, T->ptr[0]->type, 'P', 0);
            T->num = 1; //参数个数计算的初始值
            break;
        case COMP_STM:
            LEV++;
