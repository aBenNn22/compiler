...
//  %type 定义非终结符的语义值类型
%type  <ptr> program ExtDefList ExtDef Specifier VoidType ExtDecList FuncDec CompSt VarList VarDec ParamDec Stmt StmDefList Def DecList Exp Args ForArgs ForArg Term Arrays InitVal InitValMul
...
Term: ID Arrays {$$=mknode(ARRAYS_TERM,$2,NULL,NULL,yylineno);strcpy($$->type_id,$1);}
      | ID {$$=mknode(ID,NULL,NULL,NULL,yylineno);strcpy($$->type_id,$1);}
    ;

Arrays: LB Exp RB {$$=$2;}
      | LB Exp RB Arrays {$$=mknode(ARRAYS,$2,$4,NULL,yylineno); strcpy($$->type_id,"ARRAYS"); }
      ;

VarDec: Term {$$=$1;}
      | Term ASSIGN InitVal  {$$=mknode(VAR_DEC,$1,$3,NULL,yylineno);strcpy($$->type_id,$1->type_id); }
      | Term ASSIGN Exp   {$$=mknode(VAR_DEC,$1,$3,NULL,yylineno);strcpy($$->type_id,$1->type_id);}
      ;

InitVal: LC RC {$$=NULL;}
      | LC InitValMul RC {$$=NULL;}
      ;

InitValMul: Exp {$$=$1;}
        | InitValMul COMMA Exp {$$=mknode(INIT_VAL_MUL,$1,$3,NULL,yylineno);}
        ;
...
FuncDec: ID LP VarList RP  {$$=mknode(FUNC_DEC,$3,NULL,NULL,yylineno);strcpy($$->type_id,$1);}  //函数名存放在$$->type_id
       ...
       | ID LP VOID RP {$$=mknode(FUNC_DEC,NULL,NULL,NULL,yylineno);strcpy($$->type_id,$1);}  //函数名存放在$$->type_id
       ;  
...
Exp: Exp ASSIGN Exp  {$$=mknode(ASSIGN,$1,$3,NULL,yylineno);strcpy($$->type_id,"ASSIGN");}//$$结点type_id空置未用，正好存放运算符
   ...
   | ID Arrays       {$$=mknode(ARRAYS_CALL,$2,NULL,NULL,yylineno);strcpy($$->type_id,$1);}
   | ID              {$$=mknode(ID,NULL,NULL,NULL,yylineno);strcpy($$->type_id,$1);}
